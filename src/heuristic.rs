...
impl Heuristic for HungarianHeuristic {
    fn new_push(game: &Game, frozen_boxes: Bitvector) -> Self {
        let distances = Box::new(compute_push_distances(game, &frozen_boxes));
        let frozen_goals = compute_frozen_goals(game, &frozen_boxes);
        HungarianHeuristic {
            distances,
            frozen_boxes,
            frozen_goals,
...
fn compute_frozen_goals(game: &Game, frozen_boxes: &Bitvector) -> Bitvector {
...
fn compute_hungarian_heuristic(
    ...
    let box_count = game.box_count();
    let unfrozen_count = box_count - frozen_boxes.len();
    ...
    for (box_idx, &box_pos) in game.box_positions().iter().enumerate() {
        // Skip frozen boxes
        if frozen_boxes.contains(Index(box_idx as u8)) {
            continue;
        }
